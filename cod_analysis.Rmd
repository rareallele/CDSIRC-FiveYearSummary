...
exclusions <- readxl::read_xlsx("C:\\Users\\jagvan\\OneDrive - South Australia Government\\Code\\exclusions_2021.xlsx")$`Case Number`
...
data = data %>% 
  ...
  mutate(`Age Group` = 
         ...
         Supercategory = as.factor(case_when(
           `COD Category` %in% c("accident","drowning","fatal assault","fire-related",
...
(age_group = plot_ly(age_group_perc,
        type = 'bar',
        orientation = 'h',
        x = ~perc,
        y = ~fct_rev(`Age Group`),
        ...
        hovertemplate = paste0("Number of deaths: ", age_group_perc$n, "\n",
                               "Death rate: ", round(age_group_perc$rate,1), " deaths per 100,000 population", "<extra></extra>")) %>% 
...
(s1 <- subplot(plotList(2), nrows = 2 ))
s2 <- subplot(plotList(2), nrows = 2)
subplot(
  plot_ly(), s1, s2, nrows =2, 
  margin = 0.04
)
```
